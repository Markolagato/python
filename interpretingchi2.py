...
for i in xrange(2):
  ...
  for j in xrange(iters):
    ...
    # draws random values from a normal distribution with the given parameters

    resids = np.abs(yvals - 1./xvals)
    # why are we subtracting 1./xvals?
    # because the equation for residuals is (O-E) where our observerved
    # are the yvals generated from our xvals+rdmGuassVals and our expected
    #come from the "model" y=1/x

    chisq = np.sum(resids**2 / errs**2)
    # roughly, how is chisq related to N?
    # chisq should go by the magnitude of N since the resids/errs ~= 1, and 
    # we're summing over the number of data points so N*1=N

    # what if we didn't know the errors and overestimated
    # them as 0.2 -- how would chi^2 change?
    # The chisq value would decrease because res^2/err^2 if the res~=0.1 
    # would scale as 1/4
  ...
  if i==0:
      ...
      # reduced means that we have divided by narr (N) so that we are
      # approximating a "good" chisq value to be close to 1. We divide by
      # narr because chisq ~= N so chisq/N ~= 1.
...
# using different colors, a transparent plot, and different markers.
...
print(np.sum(redchisqdist1[inds])/1000) # fill in to print red. chi^2 for 99.8% conf. for N=30
inds=np.argsort(redchisqdist2)
print(np.sum(redchisqdist2[inds])/1000) # fill in to print red. chi^2 for 99.8% conf. for N=300
