...
if i == 0:
    for i in xrange(2):
      ...
      for j in xrange(iters):
        ...
        #   gaussian spread of values centered on 0 and 1 sigma = errs
    
        resids = np.abs(yvals - 1./xvals)
        # why are we subtracting 1./xvals?
        #   That is still the actual function with no errors, while yvals now
        #   has errors so this is the residuals
    
        chisq = np.sum(resids**2 / errs**2)
        # roughly, how is chisq related to N?
        #   Since the residual is based on the error and this is a sum of those
        #   N will be proportional to chisq by a factor of whatever the difference
        #   in the resids and the errors are
    
        # what if we didn't know the errors and overestimated
        # them as 0.2 -- how would chi^2 change?
        #   chisq would now be proportional to N by 0.2 so it would be much smaller
        #   than if we base it on actual errors and residuals
      ...
      if i==0:
          ...
          #     Reduced in this case mostly means normalized by the size
          #     of the data, N, this is a probability and we want it to 
          #     sum to 1.
    ...
    #   filling in histograms to make it easier to see the different trends and not
    #   mix up two similar lines.
    ...
    inds1=np.argsort(redchisqdist1)
    inds2=np.argsort(redchisqdist2)
    #print() # fill in to print red. chi^2 for 99.8% conf. for N=30
    print(redchisqdist1[inds1[998]])
    print(redchisqdist2[inds2[998]])
