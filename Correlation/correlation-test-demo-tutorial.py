...
plt.text(4,13,'rms %0.2f' % rms,size=11,color='b')
...
plt.plot(testxvals,3.+0.5*testxvals,'r',linestyle=':',linewidth=2.)
rms=(np.mean((y4 - (3 + .5 * x4))))**(1/2) # you fill in ???
plt.text(3,12,'rms %0.2f' % rms,size=11,color='b')
plt.xlim(2,20)
plt.ylim(2,14)
plt.title('garbage')
...
# added in the kendalltau stats too (same stuff pretty much)
